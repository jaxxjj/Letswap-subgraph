...
export class LetSwapFactory extends Entity {
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save LetSwapFactory entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type LetSwapFactory must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set("LetSwapFactory", id.toString(), this);
    }
  }

  static loadInBlock(id: string): LetSwapFactory | null {
    return changetype<LetSwapFactory | null>(
      store.get_in_block("LetSwapFactory", id),
    );
  }

  static load(id: string): LetSwapFactory | null {
    return changetype<LetSwapFactory | null>(store.get("LetSwapFactory", id));
  }
...
export class LetSwapDayData extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save LetSwapDayData entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type LetSwapDayData must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set("LetSwapDayData", id.toString(), this);
    }
  }

  static loadInBlock(id: string): LetSwapDayData | null {
    return changetype<LetSwapDayData | null>(
      store.get_in_block("LetSwapDayData", id),
    );
  }

  static load(id: string): LetSwapDayData | null {
    return changetype<LetSwapDayData | null>(store.get("LetSwapDayData", id));
  }
